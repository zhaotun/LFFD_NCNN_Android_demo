...
Java_com_poya_LFFD_LFFD_FaceDetectionModelInit(JNIEnv *env, jobject instance,
    ...
    //没判断是否正确导入，懒得改了
    lffdnet = new LFFD(tFaceModelDir,5);//5或8
    //mtcnn->SetMinFace(40);
...
JNIEXPORT jboolean JNICALL
Java_com_poya_LFFD_LFFD_SetThreadsNumber(JNIEnv *env, jobject instance, jint threadsNumber) {
    if(!detection_sdk_init_ok){
    ...
    lffdnet->SetNumThreads(threadsNumber);

    return  true;
}

JNIEXPORT jintArray JNICALL
Java_com_poya_LFFD_LFFD_FaceDetect(JNIEnv *env, jobject instance, jbyteArray imageDate_,
    ...
    lffdnet->detect(ncnn_img,face_lis,imageWidth,imageHeight);

    //retinafacenet->detect(ncnn_img, faceobjects);

    //int32_t num_face = static_cast<int32_t>(finalBbox.size());
    int32_t num_face = static_cast<int32_t>(face_lis.size());
    LOGD("检测到的人脸数目----：%d\n", num_face);
    ...
    for(int i=0;i<num_face;i++){
        ...
        float p = face_lis[i].score;
        LOGD(" ===faceobjects[i].prob = %f\n", p);

        faceInfo[14*i+1] = face_lis[i].x1;//left
        faceInfo[14*i+2] = face_lis[i].y1;//top
        faceInfo[14*i+3] = face_lis[i].x2;//right
        faceInfo[14*i+4] = face_lis[i].y2;//bottom

        for (int j =0;j<10;j++){
            faceInfo[14*i+5+j]=static_cast<int>(face_lis[i].landmarks[j]);
        }
    }
